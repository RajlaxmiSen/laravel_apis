...
class PhotoCompetition_Controller extends Controller
{
    ...
    public function index(Request $request) {
		...
		if ($request->ajax()) {
			return view('admin.photo_competition.grid')->with(['photos'=>$photos])->with('search_mode',$search_mode)->with('sr_dt',$sr_dt)->with('pager_data',$pager_data)->render();
		}

		return view('admin.photo_competition.list')->with(['photos'=>$photos])->with('search_mode',$search_mode)->with('sr_dt',$sr_dt)->with('pager_data',$pager_data);
	}
