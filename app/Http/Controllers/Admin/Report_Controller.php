...
class Report_Controller extends Controller
{
    ...
    public function index(Request $request) {
        ...
        $reports = $model->orderBy('id', 'desc')->paginate($limit, ['*'], 'cp');    
        $total_count = $reports->count();
        $rec_on_page = count($reports);
		...
		if ($request->ajax()) {
			return view('admin.report_issue.grid')->with(['reports'=>$reports])->with('search_mode',$search_mode)->with('sr_dt',$sr_dt)->with('pager_data',$pager_data)->render();
		}

		return view('admin.report_issue.list')->with(['reports'=>$reports])->with('search_mode',$search_mode)->with('sr_dt',$sr_dt)->with('pager_data',$pager_data);
	}
