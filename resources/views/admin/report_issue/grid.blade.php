<div>
    <div class="abl-pager">
        ...
        <div class="pull-right" style="margin-right: 10px">{{ $reports->links()}}
        </div>
</div>
<table class="table table-striped table-bordered table-hover table-checkable order-column" id="grid">
    <thead>
        <tr>
            ...
            <th width="15%"> Feed_id </th>
            <th width="10%"> Photo_id </th>
           {{--  <th width="10%"> DOB</th>
            <th width="10%"> About</th>
            <th width="5%"> Post count </th>
            <th width="5%"> Friend Count </th> --}}
...
?>
    <tbody id="tax-search-filter">
        <tr class="abl-filter-row">
            ...
            <td class="text-center">
               {{--  {!! Form::text("Name",isset($sr_dt['name'])?$sr_dt['name']:"",["class"=>"form-control abl-filter","placeholder"=>"First Name"]) !!} --}}
            </td>
            <td class="text-center">
               {{--  {!! Form::text("email",isset($sr_dt['email'])?$sr_dt['email']:"",["class"=>"form-control abl-filter ","placeholder"=>"Email"]) !!} --}}
            </td>
            <td>
                 {{-- {!! Form::text("Mobile",isset($sr_dt['mobile'])?$sr_dt['mobile']:"",["class"=>"form-control abl-filter","placeholder"=>"Mobile"]) !!} --}}
            </td>
            <td></td>
            {{-- <td></td>
            <td></td>
    ...
    <tbody>
        ...
        <tr class="odd gradeX">
            ...
            <td>{{$report->feed_id}}</td>
            <td>{{$report->photo_id}}</td>
            {{-- <td>@if($user->dob)  {{date('d-m-Y', strtotime($user->dob))}} @endif</td>
            <td>{{$user->profile->about_info}}</td>
            <td>{{$user->profile->posts_count}}</td>
            <td>{{$user->profile->friends_count}}</td> --}}
...
<div class="abl-pager">
    ...
    <div class="pull-right" style="margin-right: 10px">{{ $reports->links()}}</div>
</div>
